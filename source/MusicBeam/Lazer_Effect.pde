class Lazer_Effect extends Effect
{
  String getName() {
    return "Lazer Glow rand move";
  }
  ...
  Lazer_Effect(MusicBeam ctrl, int y)
  {
    super(ctrl, y);
  ...
  void draw()
  {
    ...
    stg.fill(00*rc[0], 100, 100);
    stg.ellipse(rx[0], ry[0], radius, radius);
    stg.fill(60*rc[0], 70, 100);
    stg.ellipse(rx[0], ry[0], (radius/1.5), (radius)/1.5);
    

    rx[0] =  rx[0]+ (delaySlider.getValue()*4);
    ry[0] = ry[0]+ (delaySlider.getValue()*4);
   ...
   /* radius = stg.getMinRadius()*radiusSlider.getValue();
    ...
    if (timer<=0 && (isTriggeredByBeat() || isTriggeredManually()))
    {
      ...
      ry[1] = ry[0];
      rc[1] = rc[0];
      rx[0] = random(radius/2, stg.width-radius/1);
      ry[0] = random(radius/2, stg.height-radius/1);
      while(rc[0]==rc[1])
        rc[0] = random(0, 8);
      timer = (delaySlider.getValue()*frameRate*3)/5;
      fader = frameRate/10;
    }
